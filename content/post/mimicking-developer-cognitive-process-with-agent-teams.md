...
that can only report back to the main agent, the agent 'teammates' can talk to each other using the `SendMessage` tool. 
...
inputs and outputs, any public contracts, impact on the current product flows etc.
...
Any contentious information bubbles up to the human, who must decide what should be built.
...
introducing some new functionality. In such situations, this agent can just document some invariants that shouldn't break 
...
At bootstrap, it loads the style guides, code conventions and all HOWTO docs on how code is organized in the project. Before starting to write code it should produce an implementation plan (IMPL) and get it reviewed by the developer.
...
as possible. Because the later we discover issues down the line, the more expensive it gets to change the code in terms of time and tokens.
...
and any information that wasn't present in the project guides. It indexes all the information and can load on demand. 
The memory feature comes with great promise that can gradually reduce the amount of management these agents need. 
However, memory bloat is also a concern. So periodic review and compaction is needed. I'm still experimenting with this 
feature to learn how well this actually works.
...
The knowledge collected by it is supposed to function like a second brain, relieving the developer from having to know or remember everything tiny detail. It provides value by:
### Gathering knowledge
It collects and updates all knowledge related to a project that is difficult to express in the code. It can document:
...
In my team, we prefer to keep this agent's documentation in the project repo itself. It might also be a good idea if the 
...
is only solving a part of the problem, it is possible to use cheaper models for them. So the overall cost may turn out to be 
comparable to using a single agent on a bigger model.
...
Before AI, there was a limit on how much planning and collaboration developers could do. Because the clock is ticking, and 
at some point, you need to stop talking and start coding. AI automates the typing, but still consumes developer time to keep 
